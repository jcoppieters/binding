...
export class TcpConnection extends EventEmitter {
  ...
  private _subscribedToCanBus: boolean = false; // Track subscription state (for debugging)
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket.on('connect', () => {
        ...
        if (this.password) {
          ...
          // Explicitly unsubscribe from CAN bus to stop receiving CAN messages
          // (Master may have subscription active from previous connection)
          console.log('Explicitly unsubscribing from CAN bus...');
   ...
   * Only required for binding uploads and direct CAN bus operations
   * NOT needed for regular unit control/status monitoring
   */
  public subscribeToCanBus(): void {
    ...
    this._subscribedToCanBus = true;
    this.sendRaw(message);
  }
  
  /**
   * Unsubscribe from CAN bus messages
   * Should be called after binding uploads to stop receiving CAN bus traffic
   */
  public unsubscribeFromCanBus(): void {
    ...
    this._subscribedToCanBus = false;
    this.sendRaw(message);
  }
  ...
  private handleMessage(bytes: number[]): void {
    ...
    if (bytes.length >= 9 && 
      ...
      // Only log responses that aren't errors or common status updates
      const verboseLog = ![0x11, 0xA9].includes(response.messageCode);
      if (verboseLog) {
      ...
      if (response.messageCode === TcpConnection.EV_MESSAGEERROR) {
        ...
        // Suppress common/expected errors to reduce console spam
        const SUPPRESS_ERRORS = [
          0x12,  // ONLY_FOR_MASTER_LCD - common when querying virtual/special units
          0x09,  // UNIT_NR_INVALID - unit doesn't exist on node
        ];
        
        if (!SUPPRESS_ERRORS.includes(errorCode)) {
          console.error(`❌ Node error (0x${response.nodeAddress.toString(16)}): ${errorName}`);
        }
    ...
    // Known but unhandled message types (suppress logging for these)
    // These are either handled by other services or are routine status messages
    const SUPPRESS_UNHANDLED = [
      0,   // RecDBInfo - database info response (handled by MasterConnectionService)
      1,   // RecNodeInfo - node info response (handled by MasterConnectionService)
      2,   // RecUnitInfo - unit info response (handled by MasterConnectionService)
      4,   // RecMood - mood status (handled by MasterConnectionService)
      5,   // RecDimmer - dimmer status (handled by MasterConnectionService)
      6,   // RecSwitch - switch status (handled by MasterConnectionService)
      7,   // RecSensor - temperature sensor status (handled by MasterConnectionService)
      38,  // RecMotor - motor/blind status (handled by MasterConnectionService)
      48,  // RecAutoStatus - automation status
      69,  // RecMacro - macro status messages
      72,  // RecHeartbeatStatus - normal keepalive response
      73,  // RecScheduleStatus - schedule status messages
    ];
    
    if (!SUPPRESS_UNHANDLED.includes(bytes[0])) {
  ...
  /**
   * Send a formatted message string
   * Format: [byte1,byte2,...]\n
   */
  send(formattedMessage: string): void {
    ...
    console.log('TX:', formattedMessage.trim());
    this.socket.write(formattedMessage);
  }
  ...
  /**
   * Check if subscribed to CAN bus
   */
  isSubscribedToCanBus(): boolean {
    return this._subscribedToCanBus;
  }
}
