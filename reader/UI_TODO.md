...
- [ ] **P0-1** Define `.duo` project file schema (`src/models/project.ts`)
  - `meta`, `railView` (cabinets → rails → modules + woningDevices), `homeView` (floors → rooms → devices + channelRefs), `bindings` (structured JSON per node)
- [ ] **P0-3** New `index.html` shell — tabs, sidebars, no inline data
  - `public/app/` : `main.js`, `state.js`, `router.js`
  - `public/components/` : one file per UI component
