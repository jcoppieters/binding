...
/*************************************************************/
class CConfigBoseStFSMMaster : public CConfigExternalFSMMaster
{
	typedef CConfigExternalFSMMaster Base;
	
	enum 
	{ 
		CONFIG_CLASS_INDEX_RENDER		= 0,
...
public:
	CConfigBoseStFSMMaster(IComApplication& parent);
	~CConfigBoseStFSMMaster( );
	BOOL Start(CConfigBoseSTCfgInfo& info);
	BOOL Cancel( );
...
protected:
	...
	BOOL startChildFSMCfgRender( );

	BOOL (CConfigBoseStFSMMaster::*m_pfStartChildFsm[CONFIG_CLASS_INDEX_MAX])( );
