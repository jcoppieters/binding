...
public:
		...
		CThreadBase()
		{ 
			m_pThreadFunc = CThreadBase::EntryPoint; // Can call Detach() also.
		}
		 ...
		 */
		CThreadBase(LPTHREAD_START_ROUTINE lpExternalRoutine)
		{
		...
		virtual DWORD Start( void* arg = NULL )
		{
			...
			//SetThreadPriority(m_ThreadCtx.m_hThread, THREAD_PRIORITY_HIGHEST);		// V1.3
		 ...
		 */
		void  Detach( void ){
			m_pThreadFunc = CThreadBase::EntryPoint;
		}
...
protected:
		...
		{
			CThreadBase *pParent = reinterpret_cast<CThreadBase*>(pArg);
			pParent->ThreadCtor();
			pParent->Run( pParent->m_ThreadCtx.m_pUserData );
			pParent->ThreadDtor();
// new ...
			if (pParent->m_ThreadCtx.m_hThread )
			{
				GetExitCodeThread(pParent->m_ThreadCtx.m_hThread, &(pParent->m_ThreadCtx.m_dwExitCode));
			}
			return pParent->m_ThreadCtx.m_dwExitCode;
