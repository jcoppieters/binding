...
m_hostAddress(std::string(hostAddress)), m_port(port), m_endpoint(std::string(endpoint)), m_bodyData(), m_headers(), 
	m_protocol(protocol), m_flags(0)  { }
...
std::string AuthHelper::CreateEncryptedBasicBase64Header(const char* str1, const char* str2,const char* secretKey) {
	...
	const int size = encrypt(plainHeader.c_str(),secretKey,arr,length);	
	if (size<=0) {
		delete [] arr;
		return std::string();	
	}
...
int AuthHelper::encrypt(const char* src,const char* key,unsigned char* arr,int maxLength) {
	if (NULL == src || NULL == key || NULL == arr) {		
		return 0;
	}
	const int stringLength = strlen(src);
	if (maxLength<stringLength) {
		ASSERT(maxLength>=stringLength);
		return 0;
	}   
