		...
		HttpRCfgResult_t errorCode, const char* msgError) 
{
	...
	if (m_ptrGui) {
		m_ptrGui->taskStatus(task, status, ec, msgError);
	}
}
...
void AppRemoteCfgMediator::taskUploadNamesProgressInfo(const AppCfgCmnProgressInfo& refInfo) {
	if (m_ptrSystemEvents) {
		m_ptrSystemEvents->taskUploadNamesProgressInfo(refInfo);
	}
}
...
bool AppRemoteCfgMediator::loadServersInfo() {	
	AppRemoteCfgServers& refServers = m_refCtx.getServers();

	// Alle huidige servers wissen.
	refServers.clear();

	// Inladen van de servers.
	const CString dir = getPathClientCredentials();
	if (!dir.IsEmpty()) {
		if (AppRemoteCfgUtils::loadServers(&refServers, (LPCSTR)dir)) {
			return true;
		}
	}
	return false;
...
bool AppRemoteCfgMediator::saveServersInfo() {
	AppRemoteCfgServers& refServers = m_refCtx.getServers();

	// Inladen van de servers.
	const CString dir = getPathClientCredentials();
	if (!dir.IsEmpty()) {
		if (AppRemoteCfgUtils::saveServers((LPCSTR)dir, refServers)) {
			return true;
		}
	}
	return false;
...
bool AppRemoteCfgMediator::clearClientCredentials() {	
	m_refCtx.getCredentials().clearClientCredentialsInfo();
	return true;
}
