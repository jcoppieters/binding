...
#include <string>
/**************************************************************/
#define USE_ENCODING_FROM_UTF8_TO_ANSI		1
/**************************************************************/
...
/************************ Helper functies *********************/
/**************************************************************/
static HttpRCfgResult_t sendGetLog(const char* remoteAddress, int port, const char* endpoint, const char* clientID, const char* accessToken, AppCfgLogInfo* response)
{
	if (0 == remoteAddress || 0 == response || 0 == endpoint) {
	...
	// De ontvangen data is een UTF-8 string.
	// De Json Parser gebruikt ANSI decoding - daarom moeten we string converteren.
#if(USE_ENCODING_FROM_UTF8_TO_ANSI == 1)
	const std::string databody = CharSetDecoderHelper::change_encoding_from_UTF8_to_ANSI(httpResponse.getDataBody());
#else
	const std::string& databody = httpResponse.getDataBody();
#endif 

	// Json Data parsen.
	JsonAppCfgLogInfo jsonParser;
	result = jsonParser.parse(response, databody);
...
// private.
static HttpRCfgResult_t sendSimple_http_GET(const char* remoteAddress, int port, const char* endpoint, const char* clientID, const char* accessToken, AppRCfgResult* response)
{
...
/**************************************************************/
HttpRCfgResult_t httprcfg_sendGetCloudFirmwareVersions(const char* remoteAddress,
	int port, AppRemoteCfgFirmwareVersionInfo* response) 
{
	...
	static const char* const endpoint = "/data/varia/firmwareUpdate.json";
	const HttpRequest::protocol_t protocolType = HttpRequest::PROTOCOL_HTTP;
	HttpResponse httpResponse;
	HttpRequest httpRequest(remoteAddress, port, endpoint, protocolType);
	httpRequest.addHeader(std::make_pair(std::string("Accept"), std::string("application/json")));
...
HttpRCfgResult_t httprcfg_sendDownloadFile(const char* url, const char* destFileName)
{
	if (0 == url || 0 == destFileName) {	
		REQUIRE(0 != url);
		REQUIRE(0 != destFileName);
		return RESULT_EC_PARAMS;
	}
	...
	const HttpRequest::protocol_t protocolType = HttpRequest::PROTOCOL_HTTP;
	HttpResponse httpResponse;
	HttpRequest httpRequest(url, protocolType);
	// httpRequest.addHeader(std::make_pair(std::string("Accept"), std::string("text/plain")));

	httpRequest.setDestFileName(destFileName);
...
static HttpRCfgResult_t sendStartBindingsAction(const char* remoteAddress, int port, const char* clientID, const char* accessToken, 
	const AppCfgBindAction& action, AppRCfgResult* response)
{
	if (0 == remoteAddress || 0 == response) {
		REQUIRE(0 != remoteAddress);
		REQUIRE(0 != response);
...
static HttpRCfgResult_t sendBindingsGetLog(const char* remoteAddress, int port, const char* clientID, const char* accessToken, AppCfgLogInfo* response) {
	...
	return sendGetLog(remoteAddress, port, endpoint, clientID, accessToken, response);
}
...
static HttpRCfgResult_t sendUploadBindFile(const char* remoteAddress,
	int port, const char* clientID, const char* accessToken, int nodeAddress, const char* fileName, AppRCfgResult* response)
{
...
HttpRCfgResult_t httprcfg_sendStartBindingsAction(const char* remoteAddress,int port,const AppCfgBindAction& action, AppRCfgResult* response) {
	return sendStartBindingsAction(remoteAddress, port, 0, 0, action, response);
}

HttpRCfgResult_t httprcfg_sendUploadBindFile(const char* remoteAddress,int port, int nodeAddress, const char* fileName, AppRCfgResult* response) {
...
static HttpRCfgResult_t sendUploadFile(const char* remoteAddress,
	int port, const char* clientID, const char* accessToken, const char* fileName, const char* destinationPath, AppRCfgResult* response)
{
	if (0 == remoteAddress || 0 == fileName || 0 == response) {
		REQUIRE(0 != remoteAddress);
		REQUIRE(0 != fileName);
...
static HttpRCfgResult_t sendDeleteFile(const char* remoteAddress, int port, 
	const char* clientID, const char* accessToken, const char* fileName, AppRCfgResult* response)
{
...
HttpRCfgResult_t httprcfg_sendUploadFile(const char* remoteAddress,
	int port, const char* fileName, const char* destinationPath, AppRCfgResult* response)
{	
...
// private.
static HttpRCfgResult_t sendStartSendNames(const char* remoteAddress,
	int port, const char* clientID, const char* accessToken,  const AppCfgRemoteActionList& listAction, AppRCfgResult* response)
{
	if (0 == remoteAddress || 0 == response) {
		REQUIRE(0 != remoteAddress);
		REQUIRE(0 != response);
	...
	const HttpRequest::protocol_t protocolType = HttpRequest::PROTOCOL_HTTP;
	static const char* const endpoint = "/rconfig/names/start";
	HttpResponse httpResponse;
	HttpRequest httpRequest(remoteAddress, port, endpoint, protocolType);
	httpRequest.addHeader(std::make_pair("Accept", "application/json"));
	...
	// Json Data body invullen.
	std::string dataBody;
	JsonRCfgActionList jsonFormatter;
	if (!jsonFormatter.format(&dataBody, listAction)) {
		return RESULT_EC_PARAMS;
	}
	httpRequest.setDataBody(dataBody.c_str());
...
static HttpRCfgResult_t sendCancelSendNames(const char* remoteAddress,int port, const char* clientID, const char* accessToken, AppRCfgResult* response) {
	static const char* const endpoint = "/rconfig/names/cancel";
	return sendSimple_http_GET(remoteAddress, port, endpoint, clientID, accessToken, response);
}

static HttpRCfgResult_t sendNamesGetLogInfo(const char* remoteAddress, int port, const char* clientID, const char* accessToken, AppCfgLogInfo* response) {
	static const char* const endpoint = "/rconfig/names/log/info";
	return sendGetLog(remoteAddress, port, endpoint, clientID, accessToken, response);
}

static HttpRCfgResult_t sendNamesGetLogErrors(const char* remoteAddress, int port, const char* clientID, const char* accessToken, AppCfgLogInfo* response) {
	static const char* const endpoint = "/rconfig/names/log/error";
	return sendGetLog(remoteAddress, port, endpoint, clientID, accessToken, response);
}

static HttpRCfgResult_t sendNamesGetProgressInfo(const char* remoteAddress, int port, const char* clientID, const char* accessToken, AppCfgCmnProgressInfo* response) {
	if (0 == remoteAddress || 0 == response) {
		REQUIRE(0 != remoteAddress);
		REQUIRE(0 != response);
		return RESULT_EC_PARAMS;
	...
	static const char* const endpoint = "/rconfig/names/progress";
	const HttpRequest::protocol_t protocolType = HttpRequest::PROTOCOL_HTTP;
	HttpResponse httpResponse;
	HttpRequest httpRequest(remoteAddress, port, endpoint, protocolType);
	httpRequest.addHeader(std::make_pair("Accept", "application/json"));
	...
	// Json data parsen.
	JsonRCfgCmnProgressInfo jsonParser;
	result = jsonParser.parse(response, httpResponse.getDataBody());
	if (RESULT_SUCCESS != result) {
		return result;
	}
...
HttpRCfgResult_t httprcfg_sendStartSendNames(const char* remoteAddress,int port, const AppCfgRemoteActionList& listAction, AppRCfgResult* response) {
	return sendStartSendNames(remoteAddress, port, 0, 0, listAction, response);
}

HttpRCfgResult_t httprcfg_sendCancelSendNames(const char* remoteAddress,int port, AppRCfgResult* response) {
	return sendCancelSendNames(remoteAddress, port, 0, 0, response);
}

HttpRCfgResult_t httprcfg_sendNamesGetLogInfo(const char* remoteAddress,int port, AppCfgLogInfo* response) {
	return sendNamesGetLogInfo(remoteAddress, port, 0, 0, response);
}
HttpRCfgResult_t httprcfg_sendNamesGetLogErrors(const char* remoteAddress, int port, AppCfgLogInfo* response) {
	return sendNamesGetLogErrors(remoteAddress, port, 0, 0, response);
}
HttpRCfgResult_t httprcfg_sendNamesGetProgressInfo(const char* remoteAddress, int port, AppCfgCmnProgressInfo* response) {
		return sendNamesGetProgressInfo(remoteAddress, port, 0, 0, response);
}
// public https

HttpRCfgResult_t httpsrcfg_sendStartSendNames(const char* remoteAddress,
	...
	AppRCfgResult* response)
{
	...
	return sendStartSendNames(remoteAddress, port, clientID, accessToken, listAction, response);
}

HttpRCfgResult_t httpsrcfg_sendCancelSendNames(const char* remoteAddress,int port, const char* clientID, const char* accessToken, AppRCfgResult* response)
{
	if (0 == clientID || 0 == accessToken) {
		REQUIRE(0 != clientID);
		REQUIRE(0 != accessToken);
	...
	return sendCancelSendNames(remoteAddress, port, clientID, accessToken, response);
}

HttpRCfgResult_t httpsrcfg_sendNamesGetLogInfo(const char* remoteAddress,int port, const char* clientID, const char* accessToken, AppCfgLogInfo* response) {
	if (0 == clientID || 0 == accessToken) {
		REQUIRE(0 != clientID);
		REQUIRE(0 != accessToken);
		return RESULT_EC_PARAMS;
	}
	return sendNamesGetLogInfo(remoteAddress, port, clientID, accessToken, response);
}

HttpRCfgResult_t httpsrcfg_sendNamesGetLogErrors(const char* remoteAddress, int port, const char* clientID, const char* accessToken, AppCfgLogInfo* response) {
	if (0 == clientID || 0 == accessToken) {
		REQUIRE(0 != clientID);
		REQUIRE(0 != accessToken);
		return RESULT_EC_PARAMS;
	}
	return sendNamesGetLogErrors(remoteAddress, port, clientID, accessToken, response);
}

HttpRCfgResult_t httpsrcfg_sendNamesGetProgressInfo(const char* remoteAddress, int port, const char* clientID, const char* accessToken, AppCfgCmnProgressInfo* response) {
	if (0 == clientID || 0 == accessToken) {
		REQUIRE(0 != clientID);
		REQUIRE(0 != accessToken);
		return RESULT_EC_PARAMS;
	}
	return sendNamesGetProgressInfo(remoteAddress, port, clientID, accessToken, response);
}
